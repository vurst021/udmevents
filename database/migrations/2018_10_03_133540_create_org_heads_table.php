...
class CreateOrgHeadsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('org_heads', function (Blueprint $table) {
            ...
            $table->foreign('org_head_userID')->references('id')
                  ->on('users')
                  ->onDelete('cascade');
            $table->integer('org_head_organizationID')->unsigned()->nullable();
            $table->foreign('org_head_organizationID')->references('org_id')
                  ->on('organizations')
                  ->onDelete('cascade');
            $table->timestamps();
        });
